...
def test_create_edge_scale_basic():
    ...
    A_mult = pd.DataFrame([[0, 2, 0], [1, 0, 3], [0, 4, 0]])
    pert_idx = [0, 1]
    ...
    expected = torch.tensor([1.0, 2.0, 4.0, 3.0, 1.0, 1.0], dtype=torch.float32)
...
def test_create_edge_scale_large_values():
    ...
    expected = torch.tensor([1e6, 1e6, 1.0], dtype=torch.float32)
