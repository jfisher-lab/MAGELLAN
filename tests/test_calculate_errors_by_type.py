...
def sample_data():
    ...
    y = pd.DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]}, index=["x", "y", "z"])

    pred1 = pd.DataFrame(
        {"A": [1.1, 2.1, 3.1], "B": [4.1, 5.1, 6.1]}, index=["x", "y", "z"]
    )

    pred2 = pd.DataFrame(
        {"A": [1.2, 2.2, 3.2], "B": [4.2, 5.2, 6.2]}, index=["x", "y", "z"]
    )

    annot = pd.DataFrame(
        {"A": ["valid", "invalid", "valid"], "B": ["valid", "valid", "invalid"]},
        index=["x", "y", "z"],
    )

    predictions = [(pred1, "pred1"), (pred2, "pred2")]
...
def test_basic_functionality(sample_data):
    ...
    errors = calculate_errors_by_type(
        predictions=predictions, y=y, annot=annot, mask_val="valid", idx_real=idx_real
    )
...
def test_empty_predictions():
    ...
    with pytest.raises(ValueError):
        calculate_errors_by_type(
            predictions=[], y=y, annot=annot, mask_val="valid", idx_real=["x", "y"]
        )
