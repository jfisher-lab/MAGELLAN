...
def simple_perturbation_dict():
    return {
        "condition1": {"pert": {"gene1": 1}, "exp": {"gene2": 0}},
        "condition2": {"pert": {"gene3": 1}, "exp": {"gene1": 0}},
    }
...
def test_nonexistent_genes():
    ...
    df = pd.DataFrame(np.zeros((2, 1)), index=["A", "B"], columns=["C1"])
    pert_dict = {"C1": {"pert": {"nonexistent": 1}, "exp": {"B": 0}}}
...
def test_nonexistent_conditions():
    ...
    df = pd.DataFrame(np.zeros((2, 1)), index=["A", "B"], columns=["C1"])
    print(df)
    pert_dict = {"nonexistent": {"pert": {"A": 1}, "exp": {"B": 0}}}
