...
def sample_data():
    ...
    pred_bound_zero_perturbed = pd.DataFrame(
        {"exp1": [0.0, 0.0, 0.5], "exp2": [0.0, 0.0, 0.8]},
        index=["node1", "node2", "node3"],
    )

    # Create sample perturbation dictionary
    pert_dic_small = {
        "exp1": {"pert": {"node1": 1.0, "node2": 2.0}, "exp": {"node3": 0.5}},
        "exp2": {"pert": {"node1": 0.0, "node2": 1.0}, "exp": {"node3": 0.8}},
    }

    # Create sample y DataFrame matching the structure
    y = pd.DataFrame(
        {"exp1": [-1, -1, 0.5], "exp2": [-1, -1, 0.8]},
        index=["node1", "node2", "node3"],
    )
...
def test_empty_perturbation_dict():
    ...
    pred_bound_zero_perturbed = pd.DataFrame(
        {"exp1": [0.0, 0.5], "exp2": [0.0, 0.8]}, index=["node1", "node2"]
    )

    pert_dic_small = {}

    y = pd.DataFrame({"exp1": [-1, 0.5], "exp2": [-1, 0.8]}, index=["node1", "node2"])
...
def test_missing_experiments():
    ...
    pred_bound_zero_perturbed = pd.DataFrame(
        {"exp1": [0.0, 0.5]}, index=["node1", "node2"]
    )

    pert_dic_small = {
        "exp1": {"pert": {"node1": 1.0}, "exp": {}},
    ...
    y = pd.DataFrame({"exp1": [-1, 0.5]}, index=["node1", "node2"])
