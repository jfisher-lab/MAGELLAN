...
def test_forward_with_round_val(sample_data):
    ...
    model = Net(edge_dim=sample_data["edge_dim"], round_val=True)
...
def test_edge_masking(sample_data):
    ...
    edge_mask = torch.tensor([1.0, 0.0, 1.0, 0.0])
...
def test_pert_mask(sample_data):
    ...
    pert_mask = torch.tensor([1.0, 0.0, 0.0, 0.0])
    ...
    masked_weights = (
        model.edge_weight.data * (torch.ones(model.edge_dim) - pert_mask) + pert_mask
    )
    assert torch.equal(
        masked_weights[pert_mask == 1], torch.ones_like(masked_weights[pert_mask == 1])
    )
...
def test_value_bounds(sample_data):
    ...
    model = Net(edge_dim=sample_data["edge_dim"], min_val=min_val, max_val=max_val)
