...
def test_train_bio_metrics_snapshot_example_1():
    ...
    cmd = ["uv", "run", "scripts/train_bio.py", "--config", str(config_path)]
    ...
    assert -1 <= binary_mcc <= 1, (
        f"Binary MCC should be between -1 and 1, got {binary_mcc}"
    )
    assert 0 <= nonbinary_f1 <= 1, (
        f"Nonbinary F1 should be between 0 and 1, got {nonbinary_f1}"
    )
    assert -1 <= nonbinary_mcc <= 1, (
        f"Nonbinary MCC should be between -1 and 1, got {nonbinary_mcc}"
    )
...
def test_train_bio_metrics_snapshot():
    ...
    config_path = (
        ...
        / "scripts"
        / "example_configs"
        / "example_train_bio_config_2.toml"
    )
