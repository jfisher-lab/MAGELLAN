...
def create_edge_weights(config, n_edges=100):
    ...
    if config.use_random_weight_init:
        if config.random_weight_init_distribution == "uniform":
            edge_weight = torch.rand(n_edges) * (config.random_weight_init_upper - config.random_weight_init_lower) + config.random_weight_init_lower
        else:
...
class TestRandomWeightInitialization:
    ...
    def test_random_initialization_statistics(self, random_config):
        ...
        expected_mean = (random_config.random_weight_init_lower + random_config.random_weight_init_upper) / 2
        ...
        expected_std = (random_config.random_weight_init_upper - random_config.random_weight_init_lower) / (12 ** 0.5)
...
class TestConfigurationValidation:
    ...
    def test_invalid_distribution(self):
        ...
        with pytest.raises(ValueError, match="Unsupported random weight initialization distribution"):
...
class TestEdgeCases:
    ...
    def test_single_edge(self, random_config):
        ...
        assert random_config.random_weight_init_lower <= edge_weight.item() <= random_config.random_weight_init_upper
    ...
    def test_train_bio_config_structure(self):
        ...
        if use_random_weight_init:
            if random_weight_init_distribution == "uniform":
                edge_weight = torch.rand(n_edges) * (random_weight_init_upper - random_weight_init_lower) + random_weight_init_lower
            else:
                raise ValueError(f"Unsupported random weight initialization distribution: {random_weight_init_distribution}")
        else:
