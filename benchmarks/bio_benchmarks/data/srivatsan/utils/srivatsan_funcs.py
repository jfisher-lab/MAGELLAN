...
def load_srivatsan_drug_table(table_path: Path) -> pd.DataFrame:
    ...
    compound_map = df[
        ["treatment", "catalog_number", "CAS.Number", "name"]
    ].drop_duplicates()
...
def classify_effect(
    row: pd.Series, significance: str = "q_value", sign: str = "normalized_effect"
) -> int:
    """
...
def get_pubchem_cid(cas_number: str) -> str | None:
    ...
    url = (
        f"https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/{cas_number}/cids/TXT"
    )
    try:
...
def get_chembl_id(cid: str) -> str | None:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            chembl_ids = [
                s for s in synonyms if isinstance(s, str) and s.startswith("CHEMBL")
            ]
            if chembl_ids:
...
def get_target_symbols(chembl_id: str) -> dict:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            print(
                f"  OpenTargets query failed for {chembl_id} (status {r.status_code})"
            )
    except Exception as e:
...
def load_mcf7_basal_perturbations(literature_spec_file: Path) -> pd.DataFrame:
    ...
    mcf7_basal = lit_spec.query(
        "experiment_particular == 'MCF7.basal' and perturbation.notna()"
    ).copy()
...
def load_lookup_tables(
    cas_to_chembl_file: Path, chembl_to_targets_file: Path, brca_json_file: Path
...
def get_perturbation_level(
    action_types: list[str], chembl_id: str, brca_max_pert_level: int
) -> int | None:
    """
...
def create_target_perturbation_rows(
    ...
    for _, row in cas_drug_map.iterrows():
        ...
        perturbation_level = get_perturbation_level(
            action_types, chembl_id, brca_max_pert_level
        )
        if perturbation_level is None:
            print(
                f"  No valid action type for {drug_name} ({chembl_id}): {action_types}"
            )
            drugs_without_targets += 1
            continue
