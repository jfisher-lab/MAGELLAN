...
def main(
    input_file: str = str(Path(__file__).parent.parent / "mcf7_brca_filtered_processed.csv"),
    output_file: str = str(Path(__file__).parent.parent / "spec" / "srivatsan_spec_master.csv"),
    ...
    brca_json_file: str = str(Path(__file__).parent.parent.parent / "BRCA" / "v1" / "json" / "kegg_shortest_path_with_phenotype.json"),
    literature_spec_file: str = str(Path(__file__).parent.parent.parent / "BRCA" / "v1" / "spec" / "literature_curated_specification.csv"),
    ...
    drug_target_rows = len(combined_df[(combined_df['perturbation'].notna()) & (combined_df['action_types'] != 'basal')])
    ...
    spec_df = pd.DataFrame({
        ...
        "experiment_particular": combined_df["drug_name"] + " (" + combined_df["cas_number"] + ")",
        ...
        "perturbation": combined_df["perturbation"].values,  # Perturbation level (0, 2, or NA)
        "expectation_bma": combined_df["expectation_bma"].values,  # Observed effect (0, 2, or NA)
        "action_types": combined_df["action_types"].values,
        "Author": "Srivatsan",
        "DEPRECATED": "N",
        "paper_code": "Srivatsan2019",
        "Figure": "",
        "mean_result": combined_df["expectation_bma"].fillna(combined_df["perturbation"]).values,
    })
    ...
    sample = spec_df.loc[spec_df['mutation.or.Amplification'] == 'basal'].head(3)[["experiment_particular", "gene", "perturbation", "expectation_bma"]]
    print(sample.to_string(index=False))

    if drug_target_rows > 0:
        print("\nSample drug target perturbation rows (first 3):")
        sample = spec_df.loc[(spec_df.loc[:, 'perturbation'].notna()) & (spec_df.loc[:, 'mutation.or.Amplification'] == 'Drug')].head(3)[["experiment_particular", "gene", "perturbation", "expectation_bma"]]
        print(sample.to_string(index=False))

    print("\nSample observation rows (first 3):")
    sample = spec_df.loc[spec_df.loc[:, 'expectation_bma'].notna()].head(3)[["experiment_particular", "gene", "perturbation", "expectation_bma"]]
    print(sample.to_string(index=False))
