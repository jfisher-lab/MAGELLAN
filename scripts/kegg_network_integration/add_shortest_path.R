...
add_phenotype_layer <- function(graph, phenotype_associations) {
  ...
  pheno_graph <- data.frame(from = phenotype_associations_df$ENTITYA,
...
phenotype_associations <- list(
  apoptosis = read.delim(file = "./data/signor_phenotype_associations/apoptosis_03_10_24.tsv", sep = "\t"),
  proliferation = read.delim(file = "./data/signor_phenotype_associations/proliferation_03_10_24.tsv", sep = "\t"),
  survival = read.delim(file = "./data/signor_phenotype_associations/survival_03_10_24.tsv", sep = "\t")
)

model_json<-read_json(path = './brca_model/model/model_files/kegg_with_phenotype.json')
g_kegg<-json_to_igraph(model_json)$graph
# Load shortest path graph
shortest_path_df <- read.csv(file = "~/Desktop/automodel/pipeline/cb_test/cb_kinase_shortest_path.csv")
...
shortestpath_with_kegg_with_pheno$g_merged <- delete_edges(shortestpath_with_kegg_with_pheno$g_merged,
...
write_json(json_keggShortestPath_withPheno,
           path = "./brca_model/model/model_files/kegg_shortest_path_with_phenotype.json", auto_unbox = TRUE)
