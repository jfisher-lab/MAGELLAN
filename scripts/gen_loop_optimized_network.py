...
def create_nested_loops(
    ...
    for cycle_idx, cycle_length in enumerate(cycle_lengths):
        ...
        while any(node in all_nodes for node in new_nodes):
            new_nodes = [
                f"{node_prefix}_{cycle_idx}_{i}_{node_counter}"
...
def analyze_loop_structure(G: Graph | nx.DiGraph) -> Dict[str, Any]:
    ...
    try:
        ...
        if cycles:
            ...
            metrics["cycle_length_distribution"] = dict(
                zip(unique_lengths.tolist(), counts.tolist())
            )
            ...
            metrics["fraction_nodes_in_cycles"] = (
                len(nodes_in_cycles) / G.number_of_nodes()
            )
    ...
    try:
        if nx.is_weakly_connected(G):
            ...
            metrics["n_strongly_connected_components"] = (
...
def generate_loop_network_configurations() -> List[Dict[str, Any]]:
    ...
    nested_configs = [
        ...
        {"cycle_lengths": [4, 5], "overlap_nodes": 2, "name": "nested_4_5_overlap2"},
        {
            "cycle_lengths": [3, 5, 4],
            "overlap_nodes": 1,
            "name": "nested_3_5_4_overlap1",
        },
        {
            "cycle_lengths": [5, 6, 7],
            "overlap_nodes": 2,
            "name": "nested_5_6_7_overlap2",
        },
    ]
    ...
    chain_configs = [
        ...
        {"cycle_lengths": [4, 4, 4], "connection_nodes": 2, "name": "chain_3x4_conn2"},
        {
            "cycle_lengths": [3, 5, 4],
            "connection_nodes": 1,
            "name": "chain_3_5_4_conn1",
        },
        {
            "cycle_lengths": [5, 6, 5, 4],
            "connection_nodes": 1,
            "name": "chain_5_6_5_4_conn1",
        },
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate loop-optimized synthetic networks"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    for config in tqdm(configurations, desc="Generating loop networks"):
        ...
        print(
            f"  Cycles: {analysis['n_cycles']}, Mean cycle length: {analysis.get('mean_cycle_length', 0):.1f}"
        )
